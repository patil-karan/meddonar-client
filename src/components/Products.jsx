...
function Products() {
  ...
  const deleteProductHandler = async (deleteId) => {
    const axiosInstance = getAxiosInstance();
    await axiosInstance
      .delete(`http://localhost:8081/api/admin/product/${deleteId}/delete`)
      .then(() => {
      ...
      <div className="items-start justify-between md:flex">
        ...
        <div className="flex gap-x-4 h-fit">
          <input
            ...
            className="w-fit px-5 py-2 outline-none border border-indigo-200 bg-indigo-50 rounded"
          />
          <select
            name="records"
            id="records"
                  ...
                  <tr key={idx}>
                    ...
                    <td className="text-center px-6 whitespace-nowrap space-x-2 gap-1">
                      <Link
                        ...
                        to={`/admin/manage-product/${item.id}`}
                      >
                        Edit
                      </Link>
                      <button
